                           ...
                           const QCString &context)
{
  ...
  while (getline(f,line))
  {
    ...
    if (qstrncmp(line.c_str(),"rect",4)==0)
    {
      ...
      char tmp_url[maxLineLen];
      sscanf(line.c_str(),"rect %s %d,%d %d,%d",tmp_url,&x1,&y1,&x2,&y2);
      url = tmp_url;

      if (urlcmp(url,"\\ref")==0 || urlcmp(url,"@ref")==0)
      {
        isRef = true;
        sscanf(line.c_str(),"rect %s %s %d,%d %d,%d",ref,tmp_url,&x1,&y1,&x2,&y2);
        url = tmp_url;
      }
